# 2026-08-15 11:02:34 by RouterOS 7.23.3
...
/ip settings
set ipv4-high-fragment-thresh=2048.0KiB max-neighbor-entries=8192
/ipv6 settings
set max-neighbor-entries=4096 min-neighbor-entries=1024 \
    soft-max-neighbor-entries=2048
/ip address
...
add disabled=no input.filter=upstream-in instance=as65001 local.address=\
    172.16.0.1 .role=ebgp name=upstream-link output.filter-chain=upstream-out \
    .network=MY-NETWORKS remote.address=172.16.0.2 .as=65002
...
add chain=upstream-in rule=\
    "rpki-verify rpki-main; if (rpki invalid) { reject } else { accept }"
/routing ospf interface-template
add area=backbone networks=10.255.0.0/30
add area=backbone networks=10.255.255.2/32 passive
/routing rpki
add address=192.168.56.1 disabled=no group=rpki-main port=3323
/snmp
