# 2026-08-07 20:02:03 by RouterOS 7.23.3
...
/ppp profile
add comment="5 Mbps residential" dns-server=10.20.0.1 local-address=\
    10.20.0.1 name=plan-5m rate-limit=5M/5M remote-address=pppoe-pool
...
add comment="shared access capacity" max-limit=20M/20M name=\
    subscriber-aggregate queue=pcq-upload/pcq-download target=\
    10.20.0.0/24
/snmp community
set [ find default=yes ] addresses=192.168.56.0/24 name=lab-ro
/interface pppoe-server server
add authentication=pap,chap default-profile=plan-5m disabled=no \
    interface=vlan100-access one-session-per-host=yes service-name=\
    SakibNet
/ip address
add address=192.168.56.10/24 comment="Host only management" interface=\
    ether2 network=192.168.56.0
/ip dhcp-client
...
/ppp secret
add comment="Residential customer 1" name=cust001 profile=plan-5m \
    service=pppoe
add comment="Residential customer 2" name=cust002 profile=plan-10m \
    service=pppoe
/snmp
...
set name=CHR-PE
/tool netwatch
add comment="uplink watchdog" down-script=":log error \"UPLINK DOWN\"" \
    host=1.1.1.1 interval=30s type=simple up-script=\
